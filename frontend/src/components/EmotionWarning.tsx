...
export default function EmotionWarning({ flag }: { flag: EmotionFlag }) {
  ...
  return (
    <div className={`relative rounded-2xl border p-5 mb-6 overflow-hidden ${
      isProhibited ? 'bg-red-500/[0.06] border-red-500/20' : 'bg-amber-500/[0.06] border-amber-500/20'
    }`}>
      {/* Decorative stripe */}
      ...
      <div className="flex items-start gap-4 mt-1">
        ...
        <div className={`w-10 h-10 rounded-xl flex items-center justify-center shrink-0 ${
          isProhibited ? 'bg-red-500/15' : 'bg-amber-500/15'
        }`}>
          {isProhibited ? (
        ...
        <div className="flex-1">
          <h3 className={`font-semibold text-base mb-1 ${isProhibited ? 'text-red-300' : 'text-amber-300'}`}>
            {isProhibited
              ? 'Article 5 Violation — Prohibited Use Case Detected'
              : 'Article 5 Warning — Emotion Recognition Detected'}
          </h3>
          <p className={`text-sm leading-relaxed ${isProhibited ? 'text-red-200/80' : 'text-amber-200/80'}`}>
            {flag.explanation}
          </p>
          {flag.context && (
            <span className={`inline-block mt-2 px-2.5 py-1 rounded-lg text-xs font-semibold ${
              isProhibited ? 'bg-red-500/15 text-red-300' : 'bg-amber-500/15 text-amber-300'
            }`}>
